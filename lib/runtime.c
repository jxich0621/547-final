...
void __sanitize_div__(int divisor, int line, int col) {
  if (divisor == 0) {
...
void __sanitize_nullptr__(void *ptr, int line, int col) {
  if (ptr == NULL) {
    printf("Null pointer dereference at line %d and col %d\n", line, col);
    exit(3);
  }
}

void __sanitize_assert__(const char *expr, int line, int col) {
  printf("[ASSERT FAIL] %s at line %d, col %d\n", expr, line, col);
  exit(4);
}
