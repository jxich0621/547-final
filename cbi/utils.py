...
def get_log_data_and_return_code_for_dir(
    target: str, input_dir: Path, expected_return_code: int = 0
) -> List[Tuple[CBILog,int]]:
    """
    ...
    log_data: List[Tuple[CBILog,int]] = list()
    for file in progress_bar:
        #  Run the target program with file
        with open(file, "rb") as fp:
            return_code = run_target(target=target, input=fp.read())

            comp_return_code = 1 if return_code != 0 else 0
            assert (
                comp_return_code == expected_return_code
            ), f"return_code {return_code} didn't match expected value: {expected_return_code}"

        if not log_file.exists():
            log_data.append(([], return_code))
        else:
            ...
            with log_save_location.open("r") as fp:
                ...
                entries = [CBILogEntry(**json.loads(log_entry)) for log_entry in fp.readlines()]
                log_data.append(
                    (entries, return_code)
                )
    return log_data
...
def get_logs_and_return_codes(target: str, fuzz_dir: Path) -> Tuple[List[Tuple[CBILog,int]], List[Tuple[CBILog,int]]]:
    """
    ...
    success_logs_and_return_codes = get_log_data_and_return_code_for_dir(
        target=target, input_dir=success_dir, expected_return_code=0
    )
    failure_logs_and_return_codes = get_log_data_and_return_code_for_dir(
        target=target, input_dir=failure_dir, expected_return_code=1
    )

    return success_logs_and_return_codes, failure_logs_and_return_codes
